...
{
    public static class Chart1Catalogue
    {
        ...
        public static void Read(string file_path)
        {
            ...
            {
                Chart1Catalogue.Read(Catalogue_Reader);
            }
            ...
            while (true)
            {
                ...
                if (Data_Sentence != null)
                {
                    if (!string.IsNullOrEmpty(Data_Sentence))
                    {
                        ...
                        if (Data_Segment.Length > 5) { Chart.Update = int.TryParse(Data_Segment[5], out int UPDN) ? UPDN : -1; }
        ...
        public static void Save_Catalogue(string file_path)
        {
            ...
            {
                foreach (KeyValuePair<string, ENC.Chart> Chart in Catalogue)
                {
                    Write_Catalogue(Writer, Chart);
                }
            }
        }
        ...
        private static void Write_Catalogue(StreamWriter writer, KeyValuePair<string, ENC.Chart> chart)
        {
            ...
            Data_Builder.Append($"{chart.Value.Boundary.South:0.#}");
