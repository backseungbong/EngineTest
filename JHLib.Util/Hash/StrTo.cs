...
namespace JHLib.Util.Hash
{
    ...
    public class StrTo<T>
    {
        private const int NULL = -1;
        private struct Entry { public string Key; public int Hash; public int Idx; }
        ...
        public string EntryKey(int i) => Ref0(_ent, i).Key;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public StrTo() => Initialize(MinPow2);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public StrTo(int cap) => Initialize(MathHelper.RoundUpToPow2(MinPow2, cap));
        ...
        {
            if (key != null && key.Length != 0 && TryGet(key, out var i)) { return Ref0(_val, i); }
            return defaultValue;
        }
